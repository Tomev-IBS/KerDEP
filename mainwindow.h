...
class MainWindow : public QMainWindow {
  ...
  private:
    ...
    const QPen DEDSTA_kde_plot_pen_ = QPen(QColor(0, 0, 255));
    const QPen DEDSTA_rare_elements_kde_plot_pen_ = QPen(Qt::black, 2);
    ...
    void DrawPlots(DEDSTA *DEDSTAAlgorithm);
    void DrawPlots(EnhancedClusterKernelAlgorithm *CKAlgorithm);
    void DrawPlots(DEDSTA_CC_WDE *WDEAlgorithm);
    void DrawPlots(SOMKEAlgorithm *somke_algorithm);
    void ClearPlot();
    void AddPlot(const QVector<qreal> *Y, const QPen &pen);
    void ResizePlot();
    unsigned long long MarkUncommonClusters(DEDSTA *DEDSTAAlgorithm);
    void MarkUncommonClusters2D(DEDSTA *DEDSTAAlgorithm, std::deque<QwtPlotCurve> *uncommon_clusters_markers);
  ...
  private slots:
    ...
    void Run1DExperimentWithDEDSTA();
    ...
    static std::vector<std::vector<double>> Generate1DPlotErrorDomain(DEDSTA *DEDSTAAlgorithm);
    static std::vector<std::vector<double>> Generate1DWindowedPlotErrorDomain(DEDSTA *DEDSTAAlgorithm);
